# RiftReview M10 screenshot capture orchestration
# Captures: phase.png (Review page, first match selected via UIAutomation)
# Verifies the "By game phase" card between the Vision card and the gold chart.
# Reuses Capturer.exe from .m2shots/Capturer/out/ and the DisableHWAcceleration registry workaround.
# Adapted from .m8shots/run_capture.ps1 -- M10 only needs the deep-dive capture.
...
$shots     = "C:\Agent Projects\RiftReview\.m10shots"
...
try {
    # SCENARIO: phase.png -- Review page, first match in demo list selected
    ...
    Write-Host ""
    Write-Host "=== SCENARIO: phase.png (--seed-demo --page review + UIAutomation select) ==="
    Kill-App
    ...
    if (-not $w2) {
        ...
        Write-Host "--- Capturing phase.png ---"
        $ok2 = Capture-Shot "$shots\phase.png"
        if ($ok2) {
            $sz2 = (Get-Item "$shots\phase.png").Length
            Write-Host "phase.png written, $sz2 bytes"
        } else {
            Write-Host "WARNING: Capturer returned non-zero for phase.png"
        }
...
Write-Host ""
Write-Host "=== M10 capture complete ==="
