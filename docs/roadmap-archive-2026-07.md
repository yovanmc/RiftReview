# RiftReview — Roadmap archive (2026-07)

Verbose shipped-milestone prose (M6–M10 decision-log detail, plus the M6 competitive-research
scoping entry) moved out of `ROADMAP.md` on 2026-07-07 to cut roadmap bloat ahead of the North
Star integration. `ROADMAP.md` keeps a condensed M1–M10 history table with pointers back here.
Nothing here is authoritative for future planning — see `NORTHSTAR.md` and the live decision log
in `ROADMAP.md` for that. This is historical record only.
...
- **2026-06-20 — M7 shipped (PR #2, all plan tasks).** Deep-dive **Vision & objectives**
  ...
  - Merge gate at the time was local `dotnet test` + the `.m?shots` screenshot subagent
    verdict only — no CI existed yet (CI landed 2026-07-02; see `ROADMAP.md` Conventions).
    `.m7shots/run_capture.ps1` reuses the M6 harness (review page, UIAutomation
    `SelectionItemPattern.Select()` first match, DisableHWAcceleration set/restore,
    `.m2shots/Capturer/out/Capturer.exe`). PNGs gitignored via `.m?shots/*.png` (harness
